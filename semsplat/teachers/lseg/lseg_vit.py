...
def _make_pretrained_clip_vitl16_384(
    ...
    clip_pretrained = nn.Module()  # text tower placeholder: unused by the dense-image path
    model = timm.create_model("vit_large_patch16_384", pretrained=pretrained)
...
def _make_vit_b32_backbone(
    ...
    # per-model activation store so multiple LSegNet instances never cross-talk
    acts: dict = {}
    ...
    pretrained.model.blocks[hooks[0]].register_forward_hook(_act_hook("1"))
    pretrained.model.blocks[hooks[1]].register_forward_hook(_act_hook("2"))
    pretrained.model.blocks[hooks[2]].register_forward_hook(_act_hook("3"))
    pretrained.model.blocks[hooks[3]].register_forward_hook(_act_hook("4"))

    pretrained.activations = acts
