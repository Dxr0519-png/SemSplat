...
class LSeg(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.text = None  # CLIP text tower removed (unused by the dense-image path)

    def forward_dense(self, x):
        """Image-only forward -> per-pixel 512-d feature map [B, 512, H, W].

        Mirrors Feature-3DGS's ``net.forward(x, return_feature=True)`` path
        (head1 -> 2x bilinear output_conv) with the CLIP text tower omitted.
        """
    ...
    def forward(self, x, labelset='', return_feature=False):
        ...
        # normalized features
        image_features = image_features / image_features.norm(dim=-1, keepdim=True) # torch.Size([57600, 512])
        text_features = text_features / text_features.norm(dim=-1, keepdim=True) # torch.Size([150, 512])
        ...
        logits_per_image = self.logit_scale * image_features.half() @ text_features.t() # torch.Size([57600, 150])
        out = logits_per_image.float().view(imshape[0], imshape[2], imshape[3], -1).permute(0,3,1,2) # torch.Size([1, 150, 240, 240])
