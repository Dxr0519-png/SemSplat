#!/usr/bin/env python
"""Five-panel gallery: 原图 / 重建图 / 真值 / reg20k / sam100k.

Compares the canonical reg run (office0_320_sam20k_reg, mIoU 0.240) against the
100k long-run (office0_320_sam100k_reg, mIoU 0.209) on office0_est320_ns. Both
prediction columns come from eval_miou_room npz dumps sharing the *identical*
``--prompts`` ordering so the 13-class palette aligns; 重建图 is re-rendered live
from ``--config`` (the 100k run's checkpoint).

Usage:
    gallery_20k_100k.py --config outputs/office0_320_sam100k_reg/.../config.yml \
        --ns data/office0_est320_ns \
        --gt-dir data/office0_est320/frames/semantic \
        --reg-npz results/orb_ate/reg_cmp/new_npz \
        --hundred-npz results/orb_ate/sam100k_cmp \
        --miou-reg results/orb_ate/reg_cmp/new_miou.txt \
        --miou-hundred results/orb_ate/sam100k_cmp/new_miou.txt \
        --frames 0 66 120 168 240 288 \
        --out results/orb_ate/sam100k_cmp/gallery
"""
...
def main() -> None:
    ...
    ap.add_argument("--config", type=Path, required=True, help="100k run config.yml (重建图列)")
    ...
    ap.add_argument("--reg-npz", type=Path, required=True, help="canonical 20k 预测 npz dir")
    ap.add_argument("--hundred-npz", type=Path, required=True, help="100k 预测 npz dir")
    ap.add_argument("--frames", type=int, nargs="+", required=True)
    ap.add_argument("--miou-reg", type=Path, default=None)
    ap.add_argument("--miou-hundred", type=Path, default=None)
    ap.add_argument("--out", type=Path, default=Path("results/orb_ate/sam100k_cmp/gallery"))
    args = ap.parse_args()
    ...
    mreg, mhund = (_miou(p) if p else None for p in (args.miou_reg, args.miou_hundred))
    iou_str = ""
    if mreg is not None:
        iou_str += f" | mIoU reg20k={mreg:.3f}"
    if mhund is not None:
        iou_str += f" | mIoU sam100k={mhund:.3f}"
    ...
    for k in args.frames:
        ...
        hcol = predcol(args.hundred_npz)

        za, zb = np.load(str(args.reg_npz / f"{k:04d}_scores.npz")), np.load(str(args.hundred_npz / f"{k:04d}_scores.npz"))
        va, vb = za["valid"].astype(bool), zb["valid"].astype(bool)
        lit = va & vb
        chg = 100.0 * (za["argmax"][lit] != zb["argmax"][lit]).mean() if lit.any() else 0.0

        imgs = [orig, recon, gtcol, regcol, hcol]
        tags = ["orig", "recon", "gt", "reg", "h100"]
        names = ["原图", "重建图", "语义真值", "reg · 20k canonical (0.240)", "samclip · 100k (0.209)"]
        cells = []
        ...
        blocks.append(
            ...
            f'<span style="color:#9aa;font-weight:400">· 20k→100k 点亮像素改判 {chg:.1f}%</span></h3>'
            f'<div style="display:flex;gap:6px">{"".join(cells)}</div></div>')
        print(f"frame {k} done", flush=True)

    title = ("office0 · 320 —— 20k canonical vs 100k 长跑 A/B：原图 / 重建图 / 语义真值 / "
             "reg20k / samclip100k")
